    ...
    def update(self):
        ...
        if self.counters:
            for counter in self.counters:
                ...
                if (counter.count == counter.range[0] or counter.count == counter.range[1]) and type(counter.value) == TempPlayer:
                    c = counter.value
                    change_variables(self, c)
                    if counter in self.counters and c.value.counters == '':
                        self.counters.remove(counter)
                        del counter

                    del c

    def temporarily_change_variables(self, t, x='', y='', width='', height='', x_off='', y_off='', width_off='', height_off='',
                 controls='', pressedValue='', speed='',
                 icon='', color='', sound='',
                 animations='', selectedAnimation='', playAnimation='', counters='',
                 xSorO='SP', ySorO='SP', widthSorO='SP', heightSorO='SP', x_offSorO='SP', y_offSorO='SP', width_offSorO='SP', height_offSorO='SP', speedSorO='SP', colorSorO='SP',
        ...
        temp = Counter(count=t, changeRange=[0, t+1], changePerSecond=1, value=TempPlayer(x=bx, y=by,
                 ...
                 animations=banimations, selectedAnimation=bselectedAnimation, playAnimation=bplayAnimation, counters=bcounters,
                 xSorO=bxSorO, ySorO=bySorO, widthSorO=bwidthSorO, heightSorO=bheightSorO, x_offSorO=bx_offSorO, y_offSorO=by_offSorO, width_offSorO=bwidth_offSorO, height_offSorO=bheight_offSorO, speedSorO=bspeedSorO, colorSorO=bcolorSorO))
        a = TempPlayer(x=x, y=y,
                    width=width, height=height, x_off=x_off, y_off=y_off, width_off=width_off, height_off=height_off,
                    controls=controls, pressedValue=pressedValue, speed=speed,
                    icon=icon, color=color, sound=sound,
                    animations=animations, selectedAnimation=selectedAnimation, playAnimation=playAnimation,
                    counters=counters,
                    xSorO=xSorO, ySorO=ySorO, widthSorO=widthSorO, heightSorO=heightSorO, x_offSorO=x_offSorO, y_offSorO=y_offSorO, width_offSorO=width_offSorO, height_offSorO=height_offSorO, speedSorO=speedSorO, colorSorO=colorSorO)

        change_variables(self, a)
        if temp not in counters:
            self.counters += [temp]
...
def change_variables(objReplaced, objReplaceWith):
    if objReplaceWith.x != '':
        if objReplaceWith.xSorO[0] == 'S':
            if objReplaceWith.xSorO[1] == 'P':
                objReplaced.x += objReplaceWith.x
            else:
                objReplaced.x *= objReplaceWith.x
        else:
            objReplaced.x = objReplaceWith.x
    if objReplaceWith.y != '':
        if objReplaceWith.ySorO[0] == 'S':
            if objReplaceWith.ySorO[1] == 'P':
                objReplaced.y += objReplaceWith.y
            else:
                objReplaced.y *= objReplaceWith.y
        else:
            objReplaced.y = objReplaceWith.y
    if objReplaceWith.width != '':
        if objReplaceWith.widthSorO[0] == 'S':
            if objReplaceWith.widthSorO[1] == 'P':
                objReplaced.width += objReplaceWith.width
            else:
                objReplaced.width *= objReplaceWith.width
        else:
            objReplaced.width = objReplaceWith.width
    if objReplaceWith.height != '':
        if objReplaceWith.heightSorO[0] == 'S':
            if objReplaceWith.heightSorO[1] == 'P':
                objReplaced.height += objReplaceWith.height
            else:
                objReplaced.height *= objReplaceWith.height
        else:
            objReplaced.height = objReplaceWith.height
    if objReplaceWith.x_off != '':
        if objReplaceWith.x_offSorO[0] == 'S':
            if objReplaceWith.x_offSorO[1] == 'P':
                objReplaced.x_off += objReplaceWith.x_off
            else:
                objReplaced.x_off *= objReplaceWith.x_off
        else:
            objReplaced.x_off = objReplaceWith.x_off
    if objReplaceWith.y_off != '':
        if objReplaceWith.y_offSorO[0] == 'S':
            if objReplaceWith.y_offSorO[1] == 'P':
                objReplaced.y_off += objReplaceWith.y_off
            else:
                objReplaced.y_off *= objReplaceWith.y_off
        else:
            objReplaced.y_off = objReplaceWith.y_off
    if objReplaceWith.width_off != '':
        if objReplaceWith.width_offSorO[0] == 'S':
            if objReplaceWith.width_offSorO[1] == 'P':
                objReplaced.width_off += objReplaceWith.width_off
            else:
                objReplaced.width_off *= objReplaceWith.width_off
        else:
            objReplaced.width_off = objReplaceWith.width_off
    if objReplaceWith.height_off != '':
        if objReplaceWith.height_offSorO[0] == 'S':
            if objReplaceWith.height_offSorO[1] == 'P':
                objReplaced.height_off += objReplaceWith.height_off
            else:
                objReplaced.height_off *= objReplaceWith.height_off
        else:
            objReplaced.height_off = objReplaceWith.height_off
    if objReplaceWith.controls != '':
        objReplaced.controls = objReplaceWith.controls
    if objReplaceWith.pressedValue != '':
        objReplaced.pressedValue = objReplaceWith.pressedValue
    if objReplaceWith.speed != '':
        if objReplaceWith.speedSorO[0] == 'S':
            if objReplaceWith.speedSorO[1] == 'P':
                objReplaced.speed = [objReplaced.speed[0] + objReplaceWith.speed[0],
                                     objReplaced.speed[1] + objReplaceWith.speed[1],
                                     objReplaced.speed[2] + objReplaceWith.speed[2],
                                     objReplaced.speed[3] + objReplaceWith.speed[3]]
            else:
                objReplaced.speed = [objReplaced.speed[0] * objReplaceWith.speed[0],
                                     objReplaced.speed[1] * objReplaceWith.speed[1],
                                     objReplaced.speed[2] * objReplaceWith.speed[2],
            ...
            objReplaced.speed = objReplaceWith.speed
    if objReplaceWith.icon != '':
        objReplaced.icon = objReplaceWith.icon
    if objReplaceWith.color != '':
        if objReplaceWith.colorSorO[0] == 'S':
            if objReplaceWith.colorSorO[1] == 'P':
                objReplaced.color = (
                objReplaced.color[0] + objReplaceWith.color[0], objReplaced.color[1] + objReplaceWith.color[1],
                objReplaced.color[2] + objReplaceWith.color[2])
            else:
                objReplaced.color = (
                objReplaced.color[0] * objReplaceWith.color[0], objReplaced.color[1] * objReplaceWith.color[1],
                objReplaced.color[2] * objReplaceWith.color[2])
        else:
            objReplaced.color = objReplaceWith.color
    if objReplaceWith.sound != '':
        objReplaced.sound = objReplaceWith.sound
    if objReplaceWith.animations != '':
        objReplaced.animations = objReplaceWith.animations
    if objReplaceWith.selectedAnimation != '':
        objReplaced.selectedAnimation = objReplaceWith.selectedAnimation
    if objReplaceWith.playAnimation != '':
        objReplaced.playAnimation = objReplaceWith.playAnimation
    if objReplaceWith.counters != '':
        objReplaced.counters = objReplaceWith.counters


if __name__ == "__main__":
    clock = pygame.time.Clock()
    man = Player(controls=[pygame.K_w, pygame.K_a, pygame.K_s, pygame.K_d], width=70, height=50)

    blok = Player(controls=[pygame.K_UP, pygame.K_LEFT, pygame.K_DOWN, pygame.K_RIGHT], width=50, height=30, color=(0, 255, 255), animations=[Animation(points=[[0, 0], [50, -10], [20, 0], [10, 20], [60, 100], [0, -5]], curve_type="BEZIER"), Animation(points=[[0, 0], [30, 15], [-15, 30], [0, 5], [1, 1]], curve_type="LINE")])
    change_variables(man, blok)
    players = [man, blok]
    drawablethings = [man, blok]
    print(man.x)
    things = [man, blok]
